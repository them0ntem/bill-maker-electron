...
export class BillService {
    ...
    updateBill(old_bill: Bill, bill: Bill, user: User): Promise<any> {
        user.money_due -= old_bill.total_amount;
        user.money_due += bill.total_amount;
        return new Promise(function (resolve, reject) {
            new UserService().updateUser(user).then(res => {
                db.update({
                    type: bill.type,
                    _id: bill._id
                }, bill, {returnUpdatedDocs: true}, function (err, numReplaced, affectedDocuments) {
                    if (err)
                        reject(err);
                    else {
                        resolve({
